...
массивов на один меньшей размерности. Можно получать длину `a.length`. Можно получать длину `a.length`.
...
Метод `.compare` сравнивает тот объект, у которого вызвали этот метод с 
...
Для корректной работы нужно, чтобы это было `.compare` было [отношением линейного
